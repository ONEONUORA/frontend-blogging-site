...
const CommentsContainer = () =>{
    let {blog, blog: {_id, title, comments:{ results : commentsArr}, activity:{total_parent_comments} } , 
     ...
     <>
            <div className={"max-sm:w-full fixed " + (commentsWrapper ? " top-0 sm: right-0" : "top-[100%] sm: right-[-100]%")  + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto  overflow-x-hidden"}>
                ...
                <div className="position-relative">
                        ...
                        <p className="text-lg mt-2 w-[70%] text-grey line-clamp-1">{title}</p>

                        <button
                             onClick={() => setCommentsWrapper(preVal => !preVal)}
                             className="position-absolute, top-0 right-0 d-flex justify-content-center items-center w-12 h-12 rounded-full bg-grey">
                             <i className="bi bi-x text-2xl mt-1"></i>
                        </button>
                </div>
                    
                    <div className="border-grey my-8 w-[120%] -ml-10" ></div>
